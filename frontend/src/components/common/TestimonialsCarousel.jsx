...
import { TESTIMONIALS as MOCK } from "../../data/mock";
import useContent from "../../hooks/useContent";

export default function TestimonialsCarousel() {
  const { data: TESTIMONIALS } = useContent("testimonials", MOCK);
  ...
  const visible = total ? Array.from({ length: Math.min(perView, total) }).map((_, i) => list[(idx + i) % total]) : [];
          ...
          <div className="md:hidden">
            <AnimatePresence mode="wait">
              <motion.article key={idx} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="bg-green-50/60 border border-green-100 rounded-2xl p-6">
                <Quote className="w-6 h-6 text-green-600 mb-3" />
                <p className="text-gray-700 text-sm leading-relaxed">{list[idx].quote}</p>
                <div className="mt-5 pt-4 border-t border-green-100">
                  <p className="font-semibold text-gray-900 text-sm">— {list[idx].name}</p>
                  <p className="text-xs text-gray-500">{list[idx].role}</p>
                </div>
              </motion.article>
            </AnimatePresence>
          </div>
